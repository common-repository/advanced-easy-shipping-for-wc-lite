( function ($) {
	...
	function commonMultiFilter ( value_based_on_cc_change, getClassName, ajaxFunction) {
			...
			jQuery(getClassName).select2({
				ajax: {
					...
					data: function (params) {
						return {
							main_cd: value_based_on_cc_change,
							value: params.term,
							action: ajaxFunction
						};
					},
	...
	function checkAllFilter () {
		jQuery( '.tr_clone .asw_sub_div .asw_condition_nt_rmd_data' ).each( function () {
			...
			if ( 'cb' === rule_name ) {
				commonMultiFilter( value_based_on_cc_change, '.asw_' + rule_name + '_' + data_sel_attr + '_condition_value_cls', 'aws_get_value_based_on_data' );
			}
			if ( 'lb' === rule_name || 'cs' === rule_name || 'us' === rule_name  ) {
	...
	function asw_get_state_list ( getSelectElement ) {
		var state_list = JSON.parse( aws_var.state_obj );
		for ( var optgroup in state_list ) {
			var $optgroup = document.createElement( 'optgroup' );
			$optgroup.setAttribute( 'label', optgroup );
			var state_option = state_list[optgroup];
			for ( var option in state_option ) {
				var $option = document.createElement( 'option' );
				$option.setAttribute( 'value', option );
				var $option_texnode = document.createTextNode( state_option[option] );
				$option.appendChild( $option_texnode );
				$optgroup.appendChild( $option );
			}
			getSelectElement.appendChild( $optgroup );
		}
		return getSelectElement;
	}
	
	function asw_get_zone_list ( getSelectElement ) {
		var zone_list = JSON.parse( aws_var.zone_obj );
		for ( var option in zone_list ) {
			var $option = document.createElement( 'option' );
			$option.setAttribute( 'value', option );
			var $option_texnode = document.createTextNode( zone_list[option] );
			$option.appendChild( $option_texnode );
	...
	function removeMinusButton () {
		var current_tr_id = jQuery( '.asw-extra-rule-div div.tr_clone' ).length;
		if (current_tr_id < 2) {
	...
	function onlyAppendTR (this_var, rule_name) {
		...
		if ('cb' === rule_name) {
			...
			selectOptionData = '';
			staticSelectAttr = 'total_qty';
			select_option_array = JSON.parse(aws_var.cart_specific_option);
		} else if ( 'us' === rule_name ) {
			selectOptionData = JSON.parse( aws_var.user_obj );
			staticSelectAttr = 'username';
			select_option_array = JSON.parse( aws_var.user_option );
		...
		if ( 'chs' == rule_name ) {
			var $main_td = document.createElement('div');
			...
			$main_td.appendChild($first_label);
		...
		/*Start First TD*/
		var $first_td = document.createElement('div');
		$first_td.setAttribute('id', 'asw_' + rule_name + '_condition_name_div_0');
		$first_td.setAttribute('class', 'asw_' + rule_name + '_condition_name_div asw_sub_div asw_row_box');
		...
		if (select_option_array) {
			for (var sel_option in select_option_array) {
				...
				if ( ( 'cc_subtotal_af_disc' != sel_option && 'cc_total_qty' != sel_option && 'cs' == rule_name ) ||
				    ( ( 'cc_country' != sel_option && 'cc_state' != sel_option && 'cc_zone' != sel_option ) && 'lb' == rule_name ) ||
				    ( ( 'cc_products' != sel_option && 'cc_categorys' != sel_option && 'cc_tags' != sel_option ) && 'cb' == rule_name ) ||
				     ( 'cc_username' != sel_option && 'us' == rule_name ) ||
		...
		if ('ps' !== rule_name) {
			/*Start Second TD*/
			var $second_td = document.createElement('div');
			$second_td.setAttribute('id', 'asw_' + rule_name + '_operator_id_0');
			$second_td.setAttribute('class', 'asw_' + rule_name + '_operator_div asw_sub_div asw_row_box');
		...
		/*Start Third TD*/
		var $third_td = document.createElement('div');
		$third_td.setAttribute('id', 'asw_' + rule_name + '_condition_value_id_0');
		$third_td.setAttribute('class', 'asw_' + rule_name + '_condition_value_div asw_sub_div asw_row_box');
		
		var selectionKey = Object.keys(select_option_array)[0];
		if ('cs' === rule_name) {
			selectionKey = Object.keys(select_option_array)[1];
		}
		aswarrayOfFieldType(selectionKey, rule_name, staticSelectAttr, '0', $third_td, selectOptionData);
		$tr.appendChild($third_td);
		/*End Third TD*/
		
		if ('ps' === rule_name) {
			/*Start TD for ps condition apply per unit*/
			var $td_apu = document.createElement('div');
			$td_apu.setAttribute('id', 'asw_' + rule_name + '_apu_id_0');
			$td_apu.setAttribute('class', 'asw_' + rule_name + '_apu_div asw_sub_div asw_row_box');
			...
			var $td_ut = document.createElement('div');
			$td_ut.setAttribute('id', 'asw_' + rule_name + '_ut_id_0');
			$td_ut.setAttribute('class', 'asw_' + rule_name + '_ut_div asw_sub_div asw_row_box');
			...
			if ($ut_data_array) {
				for (var ut_option in $ut_data_array) {
					var $option_ut_option = document.createElement('option');
					$option_ut_option.setAttribute('value', ut_option);
					if ('st_without_tax_disc' != ut_option &&
					    'weight' != ut_option) {
						$option_ut_option.setAttribute('disabled', 'disabled');
					}
					var $option_ut_option_texnode = document.createTextNode($ut_data_array[ut_option]);
					$option_ut_option.appendChild($option_ut_option_texnode);
					$td_ut_elem.appendChild($option_ut_option);
				}
			}
			...
			var $td_minval = document.createElement('div');
			$td_minval.setAttribute('id', 'asw_' + rule_name + '_minval_id_0');
			$td_minval.setAttribute('class', 'asw_' + rule_name + '_minval_div asw_sub_div asw_row_box');
			
			var $td_minval_elem = document.createElement('input');
			$td_minval_elem.setAttribute('type', 'text');
			...
			var $td_maxval = document.createElement('div');
			$td_maxval.setAttribute('id', 'asw_' + rule_name + '_maxval_id_0');
			$td_maxval.setAttribute('class', 'asw_' + rule_name + '_maxval_div asw_sub_div asw_row_box');
			
			var $td_maxval_elem = document.createElement('input');
			$td_maxval_elem.setAttribute('type', 'text');
			...
			var $td_price = document.createElement('div');
			$td_price.setAttribute('id', 'asw_' + rule_name + '_price_id_0');
			$td_price.setAttribute('class', 'asw_' + rule_name + '_price_div asw_sub_div asw_row_box');
			
			var $td_price_elem = document.createElement('input');
			$td_price_elem.setAttribute('type', 'text');
		...
		/*Start Fourth TD*/
		var $fourth_td = document.createElement('div');
	...
	function addNewTRIfBlank (this_var, rule_name, total_tr_id, new_tr_inside_table_length) {
		if (new_tr_inside_table_length == 0) {
			...
			var $tr_td = document.createElement('div');
			$tr_td.setAttribute('class', 'tbl_td');
			
			var $tr_td_table = document.createElement('div');
			$tr_td_table.setAttribute('class', 'asw-extra-rule-div');
			
			var $tr_td_table_body = document.createElement('div');
			...
			this_var.parent().parent().closest('div.row_tr').after($tr);
		}
		checkAllFilter();
	}
	...
	function checkCHKcheckedThenLoadTR () {
		jQuery('.asw-tbl-cls .asw_rule_chk_status').each(function () {
			if ($(this).is(':checked')) {
				...
				var total_tr_id = jQuery( '.asw-extra-rule-div div.row_tr' ).length;
		...
		var element_type = document.createElement(inputType);
			...
			if (inputType == 'select') {
				...
				if (selectOptionData) {
					...
					} else if ( 'state' == data_sel_attr ) {
						asw_get_state_list( element_type );
					} else if ( 'zone' == data_sel_attr ) {
						asw_get_zone_list( element_type );
					} else {
	...
	function aswarrayOfFieldType (value_based_on_cc_change, rule_name, data_sel_attr, main_tr_id, where_append_position, selectOptionData) {
		...
		if ( 'select' == type_of_field[value_based_on_cc_change] ) {
			commonMultiFilter( value_based_on_cc_change, '.asw_' + rule_name + '_' + data_sel_attr + '_condition_value_cls', 'aws_get_value_based_on_data' );
			if ( 'country' === data_sel_attr || 'state' === data_sel_attr || 'zone' === data_sel_attr || 'username' === data_sel_attr ) {
				jQuery( '.asw_' + rule_name + '_' + data_sel_attr + '_condition_value_cls' ).select2();
			}
		}
	}
	...
	var AWS_ADMIN = {
		...
		apuChecked: function () {
			...
			if ($('input[id=' + chk_id + ']').is(':checked')) {
				...
				var $td_apsub = document.createElement('div');
				$td_apsub.setAttribute('id', 'asw_' + rule_name + '_apsub_id_0');
				$td_apsub.setAttribute('class', 'asw_' + rule_name + '_apsub_div asw_sub_div asw_row_box');
		...
		showRuleSection: function () {
			...
			var new_tr_inside_table_length = jQuery( 'div[data-gb=' + rule_name + ']' ).length;
			var total_tr_id = jQuery( 'div[data-gb=' + rule_name + '] .asw-extra-rule-div div.tr_clone' ).length;
		...
		addAWSCondition: function () {
			...
			var new_tr_id = parseInt( jQuery( 'div[data-gb=' + rule_name + '] .asw-extra-rule-div div.tr_clone' ).length ) + 1;
			...
			// if ('cc_subtotal_af_disc' === check_condition_value || 'cc_country' === check_condition_value || 'cc_products' === check_condition_value) {
				...
				if ($clone.find('.asw_condition_remove').length == 0) {
					...
					create_remove_td_a.setAttribute('href', '#');
		...
		removeAWSCondition: function () {
			...
			removeMinusButton();
			jQuery( 'div[data-gb=' + rule_name + '] .asw-extra-rule-div div.tr_clone' ).each( function ( index ) {
			...
			// if ('cc_subtotal_af_disc' === value_based_on_cc_change || 'cc_country' === value_based_on_cc_change || 'cc_products' === value_based_on_cc_change) {
				...
				if ('ps' === rule_name) {
					...
					if ('cc_tc_spec' !== value_based_on_cc_change) {
						...
						var $per_unit_item_option_2 = JSON.parse(aws_var.per_unit_item_option);
						if ($per_unit_item_option_2) {
							for (var per_unit_item_2 in $per_unit_item_option_2) {
								$('#asw_' + rule_name + '_ut_id_' + main_tr_id + ' option[value=' + per_unit_item_2 + ']').remove();
							}
						}
						
						var $per_unit_cart_option_2 = JSON.parse(aws_var.per_unit_cart_option);
						if ($per_unit_cart_option_2) {
							for (var per_unit_cart_2 in $per_unit_cart_option_2) {
								var $option_per_unit = document.createElement('option');
								$option_per_unit.setAttribute('value', per_unit_cart_2);
								var $option_per_unit_texnode = document.createTextNode($per_unit_cart_option_2[per_unit_cart_2]);
								$option_per_unit.appendChild($option_per_unit_texnode);
								getSelectId.after($option_per_unit);
							}
						}
					}
				}
				...
				if ('cb' === rule_name) {
					...
					if ( 'country' === data_sel_attr ) {
						...
						selectOptionData = JSON.parse( aws_var.state_obj );
					} else if ( 'zone' === data_sel_attr ) {
						selectOptionData = JSON.parse( aws_var.zone_obj );
					} else {
						selectOptionData = '';
					}
				} else if ( 'us' === rule_name ) {
					if ( 'username' === data_sel_attr ) {
						selectOptionData = JSON.parse( aws_var.user_obj );
					} else {
						selectOptionData = '';
					}
