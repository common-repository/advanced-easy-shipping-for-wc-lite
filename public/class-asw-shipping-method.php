...
class ASW_Shipping_Method extends WC_Shipping_Method
{
    ...
    public function __construct()
    {
        ...
        $this->title = esc_html__( 'Advanced Easy Shipping', 'advanced-easy-shipping-for-woocommerce' );
        $this->method_title = esc_html__( 'Advanced Easy Shipping', 'advanced-easy-shipping-for-woocommerce' );
        $this->method_description = esc_html__( 'Configure WooCommerce Advanced Easy Shipping', 'advanced-easy-shipping-for-woocommerce' );
        $this->init();
        /**
         * Fires before shipping method init.
         */
    ...
    {
        global  $sitepress ;
        $asw_match_shipping = new ASW_Match_Shipping();
        $cart_data = WC()->cart->get_cart();
        $this->matched_methods = $this->idm_asw_check_shipping_methods( $package, $cart_data );
        ...
        if ( 'user_selection_cost' === $shipping_selection_cost || 'combine_all_cost' === $shipping_selection_cost ) {
        ...
        if ( !empty($matched_methods_arr) ) {
            foreach ( $matched_methods_arr as $method_id => $method_cost ) {
                ...
                $check_free_shipping = '';
                // Allow shipping based on date and time.
                $allow_shipping_based_on_date = 'yes';
                $allow_shipping_based_on_time = 'yes';
                
                if ( 'yes' === $allow_shipping_based_on_date || 'yes' === $allow_shipping_based_on_time ) {
                    
                    if ( !empty($asw_gss) ) {
                        $asw_shipping_tooltip = get_post_meta( $method_id, 'asw_shipping_tooltip', true );
                    ...
                    $label = ( !empty($check_free_shipping) ? $check_free_shipping : $shipping_title );
                    $this->taxable = $asw_shipping_handling_price;
                    $this->shipping_costs = $this->idm_asw_multicurrency_switcher( $method_cost );
                    /**
                     * Apply filter for shipping rate.
                     */
                    $rate = apply_filters(
                        'idm_shipping_rate',
                        array(
                        'id'        => $method_id,
                        'label'     => ( null === $label ? esc_html__( 'Shipping', 'advanced-easy-shipping-for-woocommerce' ) : $label ),
                        'cost'      => $this->shipping_costs,
                        'taxes'     => ( 'yes' === $this->taxable ? '' : false ),
                        'meta_data' => ( !empty($asw_shipping_tooltip) ? array(
                        'tooltip_description' => apply_filters( 'idm_asw_shipping_tooltip', $asw_shipping_tooltip ),
                    ) : array() ),
                        'package'   => $package,
                    ),
                        $package,
     ...
     * @param array $package   List containing all products for this method.
     *
     * @param array $cart_data Data of the cart item.
     *
     * @return array $matched_methods return all matched method.
     *
     * @since 1.0
     */
    public function idm_asw_check_shipping_methods( $package, $cart_data )
    {
        global  $sitepress ;
        ...
        foreach ( $methods as $method ) {
            $get_method_id_bol = asw_get_id_based_on_lan( $method->ID, $default_lan, ASW_POST_TYPE );
                        ...
                        foreach ( $asw_easy_shipping_data_dcd['asw_condition_name'] as $key => $asw_value ) {
                            ...
                            if ( array_key_exists( $check_rule_key, $rule_status_array ) ) {
                                foreach ( $asw_value as $value ) {
                                    ...
                                    if ( isset( $value['condition'] ) && ('cc_products' !== $value['condition'] && 'cc_categorys' !== $value['condition'] && 'cc_tags' !== $value['condition'] && 'cc_country' !== $value['condition'] && 'cc_state' !== $value['condition'] && 'cc_zone' !== $value['condition'] && 'cc_subtotal_af_disc' !== $value['condition'] && 'cc_total_qty' !== $value['condition'] && 'cc_username' !== $value['condition'] && 'cc_tc_spec' !== $value['condition']) ) {
                                        $value['condition'] = $value['condition'] . '__premium_only';
                                    } else {
            ...
            $match = $this->idm_asw_check_conitions( $new_asw_easy_shipping_arr, $package, $cart_data );
     ...
     * @since 1.0
     */
    public function idm_asw_check_conitions( $asw_easy_shipping_data_dcd = array(), $package = array(), $cart_data )
    {
        if ( empty($asw_easy_shipping_data_dcd) ) {
            return false;
        }
        ...
        if ( $asw_easy_shipping_data_dcd ) {
            ...
            if ( array_key_exists( 'rule', $asw_easy_shipping_data_dcd ) ) {
                foreach ( $asw_easy_shipping_data_dcd['rule'] as $shipping_conditions ) {
                    ...
                    /**
                     * Apply filter for match conditions.
                     */
                    $match = apply_filters(
    ...
    /**
     * Compatible with multicurrency switcher.
     *
     * @param float $price Price.
     *
     * @return mixed|void $price
     */
    public function idm_asw_multicurrency_switcher( $price )
    {
