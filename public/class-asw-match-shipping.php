...
class ASW_Match_Shipping
{
        ...
        if ( !empty($type_of_field_arr) ) {
            foreach ( $type_of_field_arr as $value ) {
                add_filter(
                    ...
                    array( $this, 'idm_asw_match_contains_fn_' . $value ),
                    10,
                    7
                );
            }
     ...
     *
     * @param array   $cart_data    Get cart data.
     *
     * @return array match result.
     *
     * @since 1.0
     */
    public function idm_asw_match_contains_fn_cc_products(
        $match,
        $operator,
        $value,
        $package,
    ...
    {
        $chk_key = 'cc_product';
        $check_array = $this->idm_get_product_id_from_packages( 'simple', $cart_data );
        $check_array = $this->asw_set_array_value_integer( 'int', $check_array );
        $check_result = $this->asw_check_rule(
            $value,
            $check_array,
            $operator,
            $chk_key,
     ...
     * @since 1.0
     */
    public function idm_asw_match_contains_fn_cc_categorys(
        $match,
        $operator,
        $value,
        $package,
    ...
    )
    {
        ...
        foreach ( $package['contents'] as $product ) {
            $cart_product_category = wp_get_post_terms( $product['product_id'], 'product_cat', array(
                'fields' => 'ids',
            ) );
     ...
     * @since 1.0
     */
    public function idm_asw_match_contains_fn_cc_tags(
        $match,
        $operator,
        $value,
        $package,
    ...
    )
    {
        ...
        $chk_key = 'cc_tag';
        foreach ( $package['contents'] as $product ) {
            $cart_product_tag = wp_get_post_terms( $product['product_id'], 'product_tag', array(
                'fields' => 'ids',
            ) );
            if ( !empty($cart_product_tag) ) {
                foreach ( $cart_product_tag as $tag_id ) {
                    $check_array[] = $tag_id;
                }
            }
        }
     ...
     * @since 1.0
     */
    public function idm_asw_match_contains_fn_cc_country(
        $match,
        $operator,
        $value,
        $package,
     ...
     * @since 1.0
     */
    public function idm_asw_match_contains_fn_cc_state(
        $match,
        $operator,
        $value,
        $package,
    ...
    )
    {
        ...
        $chk_key = 'cc_state';
        $check_array = WC()->customer->get_shipping_country() . '_' . WC()->customer->get_shipping_state();
        $check_result = $this->asw_check_rule(
            $value,
            $check_array,
            $operator,
     ...
     * @since 1.0
     */
    public function idm_asw_match_contains_fn_cc_zone(
        $match,
        $operator,
        $value,
        $package,
    ...
    )
    {
        ...
        $check_array = $shipping_zone->get_id();
        $value = $this->asw_set_array_value_integer( 'int', $value );
        $check_array = $this->asw_set_value_integer( 'int', $check_array );
        $check_result = $this->asw_check_rule(
            $value,
            $check_array,
            $operator,
    ...
    /**
     * Check rule with total cart qty.
     *
     * @param boolean $match        check rule or not.
     *
     * @param string  $operator     check operator.
     ...
     * @since 1.0
     */
    public function idm_asw_match_contains_fn_cc_total_qty(
        $match,
        $operator,
        $value,
        $package,
    ...
    )
    {
        ...
        $chk_key = 'cc_total_qty';
        $check_array = $this->idm_asw_get_total_cart_qty( $cart_data );
        $value = $this->asw_set_value_integer( 'int', $value );
        $check_result = $this->asw_check_rule(
            $check_array,
            $value,
            $operator,
     ...
     * @since 1.0
     */
    public function idm_asw_match_contains_fn_cc_subtotal_af_disc(
        $match,
        $operator,
        $value,
        $package,
     ...
     * @since 1.0
     */
    public function idm_asw_match_contains_fn_cc_username(
        $match,
        $operator,
        $value,
        $package,
    ...
    )
    {
        ...
        $chk_key = 'cc_username';
        $check_array = $this->asw_set_value_integer( 'int', $current_user_id );
        $value = $this->asw_set_array_value_integer( 'int', $value );
        /**
         *
         * $value is -$value.
     ...
     * @since 1.0
     */
    public function idm_asw_match_contains_fn_cc_tc_spec(
        $match,
        $operator,
        $value,
        $package,
    ...
    )
    {
        ...
        if ( array_key_exists( 'apu', $advanced_rule ) ) {
            ...
            if ( array_key_exists( 'ut', $advanced_rule ) ) {
                $total_per_unit_with_min_max = $this->idm_asw_get_min_max_price( $total_per_unit_with_min_max, $advanced_rule, $item = '' );
            ...
            if ( array_key_exists( 'minval', $advanced_rule ) || array_key_exists( 'maxval', $advanced_rule ) ) {
                $advance_price_total = $this->idm_check_min_max_and_add_price( $count_total_per_unit_with_min_max, $advanced_rule, $advance_price );
            }
        }
        
        /*End - Check advanced rule*/
        $match_ms_result = $this->idm_check_multiple_seletion_result( $check_result, $chk_key, 'any' );
        $match = $this->idm_asw_check_match_rule(
            $match_ms_result,
            $chk_key,
            'all',
            'advance'
     ...
     * @since 1.0
     */
    public function idm_asw_get_total_per_unit(
        $chk_key,
        $total_per_unit,
        $apsub,
        $advanced_rule,
     ...
     */
    public function asw_check_rule(
        $value,
        $check_array,
        $operator,
        $chk_key,
    ...
    )
    {
        ...
        if ( 'array' === $type ) {
            ...
            if ( !empty($value) ) {
                $check_array = $this->asw_set_array_value_integer( 'string', $check_array );
                foreach ( $value as $key => $val_id ) {
                    $val_id = $this->asw_set_value_integer( 'string', $val_id );
            ...
            if ( !empty($value) ) {
                $value = $this->asw_set_array_value_integer( 'string', $value );
                $check_array = $this->asw_set_value_integer( 'string', $check_array );
                if ( 'equal_to' === $operator ) {
    ...
    /**
     * Get matched product ids based on product type.
     *
     * @param string $prd_type  Get product type.
     *
     * @param array  $cart_data Get cart data.
     *
     * @return array
     */
    public function idm_get_product_id_from_packages( $prd_type, $cart_data )
    {
        $check_array = array();
        foreach ( $cart_data as $get_product ) {
            $get_prd_type = wc_get_product( $get_product['product_id'] );
            if ( 'simple' === $prd_type ) {
                if ( $get_prd_type->is_type( $prd_type ) && !isset( $get_product['bundled_by'] ) ) {
                    $check_array[] = $get_product['product_id'];
                }
            }
        }
        return array_unique( $check_array );
    }
    
    /**
     * Get total cart qty based on product type.
     *
     * @param array $cart_data Get cart data.
     *
     * @return string $tcq
     */
    public function idm_asw_get_total_cart_qty( $cart_data )
    {
        $tcq = 0;
        foreach ( $cart_data as $get_product ) {
            $get_prd_type = wc_get_product( $get_product['product_id'] );
            if ( $get_prd_type->is_type( 'simple' ) && !isset( $get_product['bundled_by'] ) ) {
                $tcq += $get_product['quantity'];
            }
            if ( ($get_prd_type->is_type( 'variable' ) || $get_prd_type->is_type( 'variable-subscription' )) && !isset( $get_product['bundled_by'] ) ) {
                if ( !empty($get_product['variation_id']) ) {
                    $tcq += $get_product['quantity'];
                }
            }
            if ( $get_prd_type->is_type( 'subscription' ) && !isset( $get_product['bundled_by'] ) ) {
                $tcq += $get_product['quantity'];
            }
            if ( $get_prd_type->is_type( 'bundle' ) ) {
                $tcq += $get_product['quantity'];
            }
        }
        return $tcq;
